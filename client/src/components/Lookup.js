			...
			<li className="slds-listbox__item" onClick={this.clickHandler}>
				<div id="listbox-option-unique-id-01"
                  ...
                  <span className="slds-media__figure">
                    ...
                    <span
						className="slds-listbox__option-text slds-listbox__option-text_entity">{this.props.data[this.props.labelField]}</span>
                    <span
						className="slds-listbox__option-meta slds-listbox__option-meta_entity">{this.props.data[this.props.descriptionField]}</span>
                  </span>
				</div>
			</li>
		);
...
class ListBox extends React.Component {
	...
	render() {
		let lookupItems = this.state.items.map(item => <LookupItem key={item[this.props.valueField]} data={item}
...
export default class extends React.Component {
	...
	render() {
		return (
			<div
				className={`slds-combobox slds-dropdown-trigger slds-dropdown-trigger_click slds-combobox-lookup ${this.state.opened ? 'slds-is-open' : ''}`}>
				<div className="slds-combobox__form-element slds-input-has-icon slds-input-has-icon_right">
					<InputIcon name="search"/>
					<input id="combobox" className="slds-input slds-combobox__input" type="text"
