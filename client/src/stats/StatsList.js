import React from 'react';
import moment from "moment";
import * as nav from "../services/nav";
import DataTable from "../components/DataTable";
import {DataTableFilterHelp} from "../components/DataTableFilter";

export default class extends React.Component {
	constructor(props) {
		super(props);
		this.state = {};
		
		this.linkHandler = this.linkHandler.bind(this);
	}

	// Lifecycle
	render() {
		let columns = [
			{Header: "GUS", accessor: "gus_reference", sortable: true, clickable: true},
			{Header: "Date", accessor: "date", sortable: true, clickable: true},
			{Header: "Success", accessor: "succeeded", sortable: true, clickable: true},
			{Header: "Fail", accessor: "failed", sortable: true, clickable: true},
			{Header: "Ineligible", accessor: "ineligible", sortable: true, clickable: true},
			];
		return (
			<div className="slds-color__background_gray-1">
				<DataTable id="LicenseList" keyField="sfid" columns={columns} onFetch={this.props.onFetch}
						 onClick={this.linkHandler} onFilter={this.props.onFilter} filters={this.props.filterColumns}/>
				<DataTableFilterHelp/>
			</div>
		);
	}
	
	// Handlers
	linkHandler(e, column, rowInfo) {
		switch (column.id) {
			case "name":
			case "account_name":
				return nav.toPath("license", rowInfo.original.sfid)
			case "package_name":
				return nav.toPath("package", rowInfo.original.package_id);
			case "version_sort":
				return nav.toPath("packageversion", rowInfo.original.version_id);
			default:
			// Nothing...
		}
	}
}
